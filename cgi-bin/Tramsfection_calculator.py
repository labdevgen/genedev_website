...
def print_def_form():
	...
	<form method="post" action="Transfection_calculator.py" enctype="multipart/form-data">
	...
	<b><a href="../primers.txt">Download primers database</a></b>"""
...
#	print "Parsing primers file"
	...
	for primer_name in primersdict.keys():
		...
		primer_reversed = str(Seq(primer).reverse_complement())
		position = DNA_seq.find(primer)
...
#				qualifiers_dict['ApEinfo_graphicformat']="arrow_data {{0 1 2 0 0 -1} {} 0}\nwidth 5 offset 0"
		...
		position = DNA_seq.find(primer_reversed)				
		if DNA_seq.find(primer_reversed) != -1:
			print "<tr><td>-</td><td>",primer_name,"</td><td>",primer,"</td></tr>"
			found_primer_count += 1
			if record != None:
				loc = FeatureLocation(ExactPosition(position),ExactPosition(position+20))
